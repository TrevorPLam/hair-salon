/**
 * Test suite for lib/sanitize.ts - Security-Critical Module
 *
 * These tests verify that all XSS and injection attacks are neutralized.
 * 100% coverage on security functions is REQUIRED.
 *
 * Test organization:
 * - escapeHtml: Basic escaping, attack payloads, edge cases
 * - sanitizeEmailSubject: Header injection prevention
 * - textToHtmlParagraphs: Format preservation + XSS prevention
 * - sanitizeEmail: Normalization and limits  
 * - sanitizeName: Unicode + sanitization
 * - sanitizeUrl: URL validation without unsafe protocols
 */
...
describe('lib/sanitize', () => {
  ...
  describe('sanitizeEmail()', () => {
    ...
    test('handles normal email', () => {
      expect(sanitizeEmail('john.doe+tag@example.com')).toBe(
