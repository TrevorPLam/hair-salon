...
**Duplicates/Overlap Suspected:**
- None observed in core categories (single validation stack, single styling stack).
...
- **Status:** Completed (see T-031 in `TODO_COMPLETED.md`). Registry access returned `ENETUNREACH` on 2026-01-05, even after clearing HTTP(S)_PROXY/NO_PROXY and forcing the registry via `npm install --package-lock-only` and `npm ping`. Use `npm run check:npm-registry` before the next install attempt.
...
- **Operational Guardrails:** Added `npm run check:npm-registry` to surface proxy/registry blockers before running install or lockfile tasks.
